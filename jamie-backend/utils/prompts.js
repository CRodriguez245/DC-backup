...
function getPersonaSystemPrompt(persona, stage, coachingStyle) {
    ...
    if (normalizedPersona === 'andres') {
        ...
        if (coachingStyle && exports.andresResponsePatterns) {
            ...
            const pattern = exports.andresResponsePatterns[patternKey] === null || exports.andresResponsePatterns[patternKey] === void 0 ? void 0 : exports.andresResponsePatterns[patternKey][andresStage];
            if (pattern) {
    ...
    if (normalizedPersona === 'kavya') {
        ...
        if (kavyaStage !== stage) {
            console.error(`⚠️⚠️⚠️ CRITICAL: Invalid stage "${stage}" for Kavya! Forcing "overwhelmed"`);
        }
        ...
        console.log('Kavya prompt contains "CRITICAL IDENTITY":', basePrompt.includes('CRITICAL IDENTITY'));
        // Add coaching style context if provided
